...
class Caltech256(Dataset):
	...
	def __init__(self, data_path, transformer=None, k=2):
		...
		self.k = k  # time of transformation
	...
	def __getitem__(self, idx):
		...
		if self.transformer:
			...
			imgs = [self.transformer(img) for _ in range(self.k)]
		else:
			raise ValueError("the img is supposed to be transformed to tensor")
		
		label = [label_id for i in range(self.k)]
		idx = [idx for i in range(self.k)]
...
class OfficeDataSet(Dataset):
	...
	def __init__(self, data_path, transformer=None, k=2, dataset="office_31"):
		...
		if self.dataset == "office_home":
			self.data_path = self.data_path.replace("/images", "")
			print("data_path", self.data_path)
		
		if self.dataset == "office_31":
			self.label = generate_label(self.data_path)
		self._parse_directory()
	...
	def _parse_directory(self):
		if self.dataset == "office_31":
			self.image_path = [os.path.join(root, file) for root, _, filenames in os.walk(self.data_path) for file in
			                   filenames if ".DS_Store" not in file]
			self.label_path = [root.split("/")[-1] for root, _, filenames in os.walk(self.data_path) for file in
			                   filenames if ".DS_Store" not in file]
		elif self.dataset == "office_home":
			with open(self.data_path, 'r')as txt_f:
				data_label = txt_f.readlines()
			self.image_path = [os.path.join("/users/leo/Datasets/DA/OfficeHome", i.split(" ")[0]) for i in data_label]
			self.label_path = [int(i.split(" ")[1].replace("/n", "")) for i in data_label]
		# print(self.label_path[0:5])
		if self.dataset == "office_31":
			self.image_path = sorted(self.image_path)
			self.label_path = sorted(self.label_path)
			self.image_path = [i for i in self.image_path if i]
			self.label_path = [i for i in self.label_path if i]
			self.label_path = [self.label[i] for i in self.label_path]
